...
export default function RaterCommentsPage() {
  ...
  const loadEvaluation = async () => {
    try {
      ...
      if (data.rater_comments) {
        setComments(data.rater_comments);
      } else {
  ...
  const generateComments = async (evalData?: any) => {
    ...
    try {
      ...
      const response = await fetch("/api/ai/rater", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bullets: data.categorized_bullets || [],
          rankLevel: data.rank_level,
          dutyTitle: data.duty_title,
  ...
  const handleSaveAndContinue = async () => {
    if (!comments.trim()) {
      alert("Please add rater comments.");
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase
        .from("evaluations")
        .update({
          rater_comments: comments,
          status: "rater_complete",
        })
        .eq("id", evaluationId);

      if (error) throw error;

      router.push(`/evaluation/${evaluationId}/senior-rater`);
    } catch (error) {
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-8">
          ...
          <p className="text-gray-400">
            AI-generated comments based on your categorized bullets. Edit as needed.
          </p>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-blue-400">
                  AI is Generating Rater Comments...
                </h3>
                <p className="text-sm text-gray-400">
                  Analyzing your bullets to create comprehensive rater comments
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mb-6 rounded-xl border border-white/10 bg-white/5 p-6">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold">Rater Comments</h3>
            </div>
            <button
        ...
        <div className="mb-6 rounded-lg border border-white/10 bg-white/5 p-4">
          ...
          <ul className="space-y-2 text-sm text-gray-400">
            <li className="flex items-start gap-2">
              ...
              <span>Synthesize key accomplishments from bullets</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-blue-400">•</span>
              <span>Use professional military evaluation language</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-blue-400">•</span>
              <span>Keep to 100-150 words</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-blue-400">•</span>
              <span>Be specific and results-oriented</span>
            </li>
          </ul>
        </div>
        ...
        >
          {saving ? "Saving..." : "Continue to Senior Rater Comments"}
