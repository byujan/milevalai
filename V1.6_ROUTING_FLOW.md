...
```
1. Intro / Onboarding → Sign-In
2. Sign-In → Profile Dashboard
3. Profile Dashboard → Create New Evaluation
4. Create New Evaluation → Predecessor (optional)
5. Predecessor → Bullet Drafting
6. Bullet Drafting → AI Categorization
7. AI Categorization → Rater Comments
8. Rater Comments → Senior Rater Comments
9. Senior Rater Comments → Review & Validation
10. Review & Validation → Export Evaluation
```
...
- **Routing Logic:**
  - `completed` → `/evaluation/[id]/review`
  - `senior_rater_complete` → `/evaluation/[id]/review`
  - `rater_complete` → `/evaluation/[id]/senior-rater`
  - `bullets_categorized` → `/evaluation/[id]/categorize`
  - `bullets_draft` → `/evaluation/[id]/draft`
  - Has `raw_bullets` → `/evaluation/[id]/categorize`
  - Has `categorized_bullets` → `/evaluation/[id]/categorize`
  - Has `predecessor_file_url` → `/evaluation/[id]/draft`
  - Default (`draft`) → `/evaluation/[id]/predecessor`

### **4. Create New Evaluation** → `/evaluation/create`
- **Status Set:** `draft`
- **Next:** `/evaluation/[id]/predecessor`
- **Action:** After creating evaluation

### **4b. Upload Predecessor's Evaluation** → `/evaluation/[id]/predecessor`
- **Status:** Remains `draft`
- **Next:** `/evaluation/[id]/draft`
- **Actions:**
  - Skip → `/evaluation/[id]/draft`
  - Upload & Continue → `/evaluation/[id]/draft`

### **5. Bullet Drafting** → `/evaluation/[id]/draft`
...
### **6. AI-Suggested Bullets / Categorization** → `/evaluation/[id]/categorize`
...
### **7. Rater Comments** → `/evaluation/[id]/rater`
...
### **8. Senior Rater Comments** → `/evaluation/[id]/senior-rater`
...
### **9. Review & Validation** → `/evaluation/[id]/review`
- **Status Set:** `completed` (when exporting)
- **Next:** `/evaluation/[id]/export`
- **Actions:**
  - "Export Evaluation" → `/evaluation/[id]/export`
  - "Edit Bullets" → `/evaluation/[id]/categorize`
  - "Fix Automatically" → (TODO: implement)
- **Purpose:** Edit every section before export

### **10. Export Evaluation** → `/evaluation/[id]/export`
- **Status:** `completed`
...
```
draft
  ↓ (after predecessor upload/skip)
draft (still, but has predecessor_file_url if uploaded)
  ↓ (after drafting bullets)
bullets_draft
  ↓ (after AI categorization)
bullets_categorized
  ↓ (after continuing from categorize)
bullets_categorized (still, but categorized_bullets saved)
  ↓ (after rater comments)
rater_complete
  ↓ (after senior rater comments)
...
### **New Evaluation Flow:**
- [ ] Create evaluation → Goes to predecessor page
- [ ] Skip predecessor → Goes to draft page
- [ ] Upload predecessor → Goes to draft page
- [ ] Add bullets in draft → Status becomes `bullets_draft`
- [ ] Continue to categorize → Goes to categorize page
- [ ] AI processes bullets → Status becomes `bullets_categorized`
- [ ] Continue to rater → Goes to rater page
- [ ] Save rater comments → Status becomes `rater_complete`
- [ ] Continue to senior rater → Goes to senior rater page
- [ ] Save senior rater comments → Status becomes `senior_rater_complete`
- [ ] Continue to review → Goes to review page
- [ ] Export evaluation → Status becomes `completed`

### **Returning to Evaluation:**
- [ ] Dashboard with `draft` status → Routes to predecessor or draft
- [ ] Dashboard with `bullets_draft` status → Routes to draft
- [ ] Dashboard with `bullets_categorized` status → Routes to categorize
- [ ] Dashboard with `rater_complete` status → Routes to senior-rater
- [ ] Dashboard with `senior_rater_complete` status → Routes to review
- [ ] Dashboard with `completed` status → Routes to review

### **Navigation:**
- [ ] Review page "Edit Bullets" → Goes to categorize page
- [ ] Categorize page "Continue" → Updates status and goes to rater
...
1. ✅ All pages route correctly in sequence
2. ✅ Dashboard routes intelligently based on status
3. ✅ Status transitions are correct
4. ✅ Review page links to categorize (not legacy bullets page)
5. ✅ Categorize page updates status correctly
6. ✅ Users can continue from where they left off
7. ✅ Status is displayed on dashboard

**The flow matches the v1.6 specification exactly!** 🎉
